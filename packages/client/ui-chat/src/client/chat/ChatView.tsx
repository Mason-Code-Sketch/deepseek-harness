...
export function ChatView({
  ...
  useLayoutEffect(() => {
    ...
    // A reader who stopped inside the threshold keeps the gap they created: the
    // tip only follows while the recorded distance to the floor is at most 1px.
    const canFollowFlow = atBottomRef.current && readerBottomGapRef.current <= 1
    if (appendedUser || appendedSteering || appendedSubmission || (tipMoved && canFollowFlow)) {
      toBottom(el)
      return
    }
  ...
  followRef.current = () => {
    ...
    if (local !== null) {
      const el = scrollerOf(local)
      if (!atBottomRef.current || readerBottomGapRef.current > 1) return
      el.scrollTop = el.scrollHeight
      observedTopRef.current = el.scrollTop
      readerBottomGapRef.current = 0
      chatScroll.save(null)
