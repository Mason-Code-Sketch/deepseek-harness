...
export function deriveGroups(
  ...
  for (const g of groupByWorkspace(list, workspaces, archived, view.ungroupedOrder)) {
    ...
    groups.push({
      ...
      sessions: expanded
        ? members.map(session => sessionNode(session, descendants, statuses, false))
        : [],
    })
  }
  if (pinnedMembers.length > 0) {
    ...
    const pinnedById = new Map(pinnedMembers.map(session => [session.id, session]))
    const pinnedSessions = orderByRecency(pinnedMembers.map(session => session.id), list.byId)
      .flatMap((id) => {
        const session = pinnedById.get(id)
        /* v8 ignore next -- ids are projected exclusively from the members used to build pinnedById. */
        return session === undefined ? [] : [session]
      })
    groups.unshift({
      key: PINNED_KEY,
      ...
      pinnedSection: true,
      sessions: pinnedSessions.map(session => sessionNode(session, descendants, statuses, true)),
    })
  }
  return groups
}
...
export function deriveFlat(
  ...
  return sessionIds
    .map(id => sessionNode(list.byId[id] as SessionSummary, descendants, statuses, false))
}
