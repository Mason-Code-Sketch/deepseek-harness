...
import {
  ...
  partitionPinned, pinCurrentBlank, reconcileManualOrder, UNGROUPED_KEY, visibleSessionIds,
} from '../tree.ts'
...
function SessionTree({
  ...
  const groups = useMemo(
    ...
    [list, workspaces, archivedSessionIds, statuses, expandedGroups, ungroupedSessionIds, pinnedSessionIds],
  )
  useEffect(() => {
...
> & {
  ...
  const statuses = useSessionStatus(s => s)
  const rows = useMemo(() => {
    // Pins float rows without rewriting the account order: the partition
    // applies to the caller's already-ordered visible ids, so unpinning
    // restores the exact prior arrangement.
    const pinned = new Set(pinnedSessionIds)
    return partitionPinned(deriveFlat(list, sessionIds, statuses), pinned)
      .map(row => ({ ...row, pinned: pinned.has(row.id as string) }))
  }, [list, pinnedSessionIds, sessionIds, statuses])
  const [drag, setDrag] = useState<DragState | null>(null)
...
export function WorkspaceBrowser({
  ...
  useEffect(() => {
    ...
    actions.retainPinnedSessions(list.ids.map(id => id as string))
  }, [actions.retainPinnedSessions, list.ids, list.phase])
  useEffect(() => {
