...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
  ...
  it('reveals a clipped session title by scrolling it while the row is hovered', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
    render(
  ...
  it('keeps the active-Schedule marker between the title and time in grouped and flat rows', () => {
    ...
    const node: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, hasActiveSchedule: true, updatedAt: 0, pinned: false,
    }
    const view = render(
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false,
        hasActiveSchedule: false, updatedAt: 0, pinned: false,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, hasActiveSchedule: false, updatedAt: 0, pinned: false,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
  ...
  it('shows one hover pin button on every row: grey to pin, brand blue to unpin', () => {
    const onTogglePin = vi.fn()
    const node: SessionNode = {
      id: sid('s1'), title: 'Pin me', blank: false, running: false,
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
    ...
    // Unpinned rows offer the grey one-tap pin button; the menu has no pin entry.
    const pin = screen.getByRole('button', { name: '置顶会话' })
    expect(pin.getAttribute('aria-pressed')).toBe('false')
    fireEvent.click(pin)
    expect(onTogglePin).toHaveBeenCalledWith(sid('s1'))

    // Pinned rows show the same hover button in brand blue: clicking unpins.
    rerender(
      <SessionNodeItem node={{ ...node, pinned: true }} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={onTogglePin} t={t} />,
    )
    const unpin = screen.getByRole('button', { name: '取消置顶' })
    expect(unpin.getAttribute('aria-pressed')).toBe('true')
    expect(unpin.className).toMatch(/pinActive/)
    fireEvent.click(unpin)
    expect(onTogglePin).toHaveBeenCalledTimes(2)
    // The row menu stays rename/fork/archive — no duplicate unpin entry.
    fireEvent.click(screen.getByRole('button', { name: '会话“Pin me”的操作' }))
    expect(screen.queryByRole('menuitem', { name: '取消置顶' })).toBeNull()
  })

  it('hides the pin control when no toggle is wired or the row is blank', () => {
    const node: SessionNode = {
      id: sid('s1'), title: 'Plain', blank: false, running: false,
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    ...
    const blank: SessionNode = {
      id: sid('blank'), title: 'New Session', blank: true, running: false,
      runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0, pinned: false,
    }
    render(<SessionNodeItem node={blank} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} t={t} />)
    expect(screen.queryByRole('button', { name: '置顶会话' })).toBeNull()
  })
})
